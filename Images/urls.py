...
urlpatterns = [
    ...
    path('<int:image_id>/',views.ImageDetail.as_view(),name='image_detail'),#이미지디테일
    path('likes/<int:image_id>/',views.LikeImage.as_view(),name='like_image'), #좋아요
    path('unlikes/<int:image_id>/',views.UnLikeImage.as_view(),name='unlike_image'), #좋아요취소
    path('comment/<int:image_id>/',views.CommentOnImage.as_view(),name='comment_image'), #댓글생성
    path('comment/<int:image_id>/delete/<int:comment_id>/',views.ModerateComment.as_view(),name='moderate_comment'), #댓글생성    
